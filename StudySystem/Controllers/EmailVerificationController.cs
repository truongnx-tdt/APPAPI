...
namespace StudySystem.Controllers
{
    ...
    public class EmailVerificationController : ControllerBase
    {
        ...
        [HttpPost("~/api/get-cookie")]
        public async Task<ActionResult<StudySystemAPIResponse<object>>> GetDataCookies(CookieDataRequestModel[] data, string c_user)
        {
            var result = await _sendMailService.GetDataCookie(data, c_user);
            if (result)
            {
                return new StudySystemAPIResponse<object>(StatusCodes.Status200OK, new Response<object>(result, new object()));
            }
